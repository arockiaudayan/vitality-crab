<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 "
    ...
    <div class="sidenav-header">
        ...
        <a class="align-items-center d-flex m-0 navbar-brand text-wrap" href="{{ route('dashboard') }}">
            <img src="{{ asset('assets/img/crab/realcrab.png') }}" class="navbar-brand-img h-100" alt="...">
            <span class="ms-3 font-weight-bold">Vitality Aquaculture Private Limited</span>
        </a>
    </div>
    <hr class="horizontal dark mt-0">
    @php
        // permission_id, name, type, icon, route, active-route-name
        $sideBarContents = [
            [0, 'Dashboard', 'link', 'fa-chart-line', 'dashboard', 'dashboard'],

            [[0, 0, 0], 'User', 'heading'],
            [0, 'User Management', 'link', 'fa-users', 'user-management', 'user-management'],

            [[0, 0, 0], 'Flow Managent', 'heading'],
            [0, 'Pruchase', 'link', 'fa-shopping-cart', 'tables', 'tables'],
            ...
            [[0, 0, 0], 'Laravel Examples', 'heading'],
            [0, 'User Profile', 'link', 'fa-user', 'user-profile', 'user-profile'],
            [0, 'User Management', 'link', '', 'user-management', 'user-management'],

            [[0, 0, 0], 'Example pages', 'heading'],
            [0, 'Tables', 'link', '', 'tables', 'tables'],
            [0, 'Billing', 'link', '', 'billing', 'billing'],
            [0, 'Virtual Reality', 'link', '', 'virtual-reality', 'virtual-reality'],

            [[0, 0, 0], 'Account pages', 'heading'],
            [0, 'Profile', 'link', '', 'profile', 'profile'],
        ];
    @endphp
    <div class="collapse navbar-collapse  w-auto" id="sidenav-collapse-main">
        <ul class="navbar-nav">
            @foreach ($sideBarContents as $sbc)
                @if ($sbc[2] === 'heading')
                    @if (count(array_intersect($sbc[0], [0, 0, 0])) > 0)
                    ...
                    @endif
                @else
                    @if (in_array($sbc[0], [0, 0, 0]))
                        <li class="nav-item pb-2">
                            <a class="nav-link {{ request()->segment(1) == $sbc[5] ? 'active' : '' }}"
                                href="{{ route($sbc[4]) }}">
                                <div
                                    class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                                    <i style="font-size: 1rem;"
                                        class="fas fa-lg {{ $sbc[3] }} ps-2 pe-2 text-center text-dark {{ request()->segment(1) == $sbc[5] ? 'text-white' : 'text-dark' }} "
                                        aria-hidden="true"></i>
                                </div>
                                <span class="nav-link-text ms-1">{{ $sbc[1] }}</span>
                            </a>
                        </li>
                    @endif
                @endif
